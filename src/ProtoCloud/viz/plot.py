...
def plot_latent_embedding(latent_embedding, proto_embedding=None, 
    ...
    emb_first = np.concatenate((latent_embedding[:, :half_dim], proto_embedding[:, :half_dim]), axis=0)
    umap_first = get_UMAP(emb_first)
    pc_umap1, proto_umap1 = np.split(umap_first, [latent_embedding.shape[0]], axis=0)

    emb_second = np.concatenate((latent_embedding[:, half_dim:], proto_embedding[:, half_dim:]), axis=0)
    umap_second = get_UMAP(emb_second)
    pc_umap2, proto_umap2 = np.split(umap_second, [latent_embedding.shape[0]], axis=0)
    ...
    if type_color is None:
        type_color = get_color(model_labels)

    
    if adata is None or 'batch' not in adata.obs.columns:
        fig, axs = plt.subplots(1, 2, figsize=(5, 6), 
                                gridspec_kw={'wspace': 0, 'hspace': 0}, 
                                sharex=True, sharey=True)
        axs = axs.flatten()
        # 1st-half, celltype
        plot_umap(pc_umap1, cell_labels, type_color=type_color,
                a=1, s=20, ax=axs[0])
        plot_protos_umap(proto_umap1, model_labels,
                        prototypes_per_class=6, s=30,
                        type_color=type_color, ax=axs[0])
        ...
        be1 = calculate_batch_entropy(latent_embedding[:, :half_dim], batch_labels)
        print(f"First Half Batch Entropy: {be1}")
        be2 = calculate_batch_entropy(latent_embedding[:, half_dim:], batch_labels)
        print(f"Second Half Batch Entropy: {be2}")
        ...
        plot_protos_umap(proto_umap1, model_labels,
                        ...
                        type_color=type_color, ax=axs[0])
        # 1st-half, Batch
        adata.obsm['X_umap'] = pc_umap1
        sc.pl.umap(adata, color='batch', size=20, alpha=0.7,
                ax=axs[1], title='', show=False)
...
def plot_prp_dist(celltypes, gene_names, 
    ...
    for i in range(num_classes):
        ...
        fig, axes = plt.subplots(prototypes_per_class, 1, 
                                 figsize=(20, 6*prototypes_per_class))
        ...
        for p in range(prototypes_per_class):
            ...
            for j, value in enumerate(top_genes_indices):
                ax.annotate(top_genes[j], (value, gene_rel[value]),
                            textcoords="offset points", xytext=(0, 10), ha='center', fontsize=14)
            
            if np.min(gene_rel) < 0:
                bottom_genes_indices = df.sum().nsmallest(20).index.tolist()
                bottom_genes = gene_names[bottom_genes_indices].tolist()
                for j, value in enumerate(bottom_genes_indices):
                    ax.annotate(bottom_genes[j], (value, gene_rel[value]),
                                textcoords="offset points", xytext=(0, -10), ha='center', fontsize=14)
...
def plot_gene_relevance(celltypes, gene_names, prp_path, **kwargs):
    """
    Plot the histogram of PRP scores of all genes for each cell type
    """
    class_rel = pd.read_csv(prp_path + "celltype_PRP.csv", index_col=0)
    num_classes = len(celltypes)
    fig, axes = plt.subplots(num_classes, 1, 
                            figsize=(20, 6*num_classes))

    cls_hrg = {}
    for i in range(num_classes):
        ax = axes[i]
        celltype = celltypes[i]
        gene_rel = class_rel.loc[celltype]
        
        # Plot per prototype on subplot
        ax.plot(list(range(len(gene_rel))), gene_rel)
        ax.set_title("{}: Gene relevance".format(celltype), fontsize=10)
        ax.set_ylabel('Relevance Score', fontsize=8)
        ax.set_xlabel('Gene Indices', fontsize=8)
        
        # Identify and annotate top genes
        df = pd.DataFrame(data=gene_rel.values).T
        top_genes_indices = df.sum().nlargest(20).index.tolist()
        top_genes = gene_names[top_genes_indices].tolist()
        cls_hrg[celltype] = top_genes
        for j, value in enumerate(top_genes_indices):
            ax.annotate(top_genes[j], (value, gene_rel[value]),
    ...
    plt.savefig(prp_path + 'gene_relevance.pdf', bbox_inches='tight')
    plt.close()
    
    with open(prp_path + "cls_hrg.txt", "w", encoding="utf-8") as f:
        for k, v in cls_hrg.items():
            f.write(f"{k}: {v}\n")
            
    print("\tGene relevance plots saved")


def plot_gene_expression(adata, celltypes, gene_names, plot_dir, **kwargs):
    """
    Plot the histogram of PRP scores of all genes for each cell type
    """
    num_classes = len(celltypes)
    ...
    for i in range(num_classes):
        ...
        ax.set_title("{}: Gene expression".format(celltypes[i]), fontsize=10)
        ax.set_ylabel('UMI', fontsize=8)
        ax.set_xlabel('Gene Indices', fontsize=8)
        
        # Identify and annotate top genes
        df = pd.DataFrame(data=gene_expr).T
        top_genes_indices = df.sum().nlargest(20).index.tolist()
        top_genes = gene_names[top_genes_indices].tolist()
        for j, value in enumerate(top_genes_indices):
            ax.annotate(top_genes[j], (value, gene_expr[value]),
                        textcoords="offset points", xytext=(0, 10), ha='center', fontsize=14)    
        # ax = plt.gca()
    ...
    plt.savefig(plot_dir + 'gene_expression.pdf', bbox_inches='tight')
    plt.close()
    print("\tGene expression plots saved")


def plot_gene_rel_VS_expr(adata, celltypes, gene_names, prp_path, **kwargs):
    """
    Plot the histogram of PRP scores of all genes for each cell type
    """
    class_rel = pd.read_csv(prp_path + "celltype_PRP.csv", index_col=0)
    num_classes = len(celltypes)
    n_rows = math.ceil(num_classes / 3)
    
    fig, axes = plt.subplots(n_rows, 3, figsize=(n_rows*4, 3*4))
    axes = axes.flatten() 
    ...
    for i in range(num_classes):
        ...
        top_genes_indices = df.sum().nlargest(top_n).index.tolist()
        is_top = np.zeros(len(gene_expr), dtype=bool)
        is_top[top_genes_indices] = True
        
        ax.scatter(gene_rel[~is_top], gene_expr[~is_top], 
                c='gray', s=10, label='Other genes')
        ax.scatter(gene_rel[is_top], gene_expr[is_top], 
                c='red', s=30, label=f'Top {top_n} genes')
        
        gene_names = class_rel.columns.tolist()
        for idx in top_genes_indices:
            ax.annotate(gene_names[idx], 
                        xy=(gene_rel.iloc[idx], gene_expr[idx]),
                        xytext=(5, 5), textcoords='offset points',
                        fontsize=6, alpha=0.8)
        ax.set_title(celltype)
        ax.set_xlabel('Gene Relevance')
        ax.set_ylabel('UMI')

    plt.tight_layout()
    plt.savefig(prp_path + 'gene_rel_VS_expr.pdf', bbox_inches='tight')
    plt.close()
    print("\tGene expression VS relevance plots saved")




def plot_all_class_lrp_dist(celltypes, gene_names, 
                num_classes, lrp_path, **kwargs):
    """
    Plot the histogram of PRP scores of all genes for each cell type
    """
    filename = 'zmu_lrp_relgenes.npy'
    if os.path.exists(lrp_path + filename):
        proto_rel = np.load(lrp_path + filename, allow_pickle = True)
        
    fig, axes = plt.subplots(num_classes, 1, 
                            figsize=(20, 6*num_classes))

    for i in range(num_classes):
        gene_rel = proto_rel[i, :]
        ax = axes[i]

        # Plot per prototype on subplot
        ax.plot(list(range(len(gene_rel))), gene_rel)
        ax.set_title(celltypes[i], fontsize=10)
        ax.set_ylabel('Relevance Score', fontsize=8)
        ax.set_xlabel('Gene Indices', fontsize=8)
    ...
    plt.savefig(lrp_path + 'z_mu_lrp.pdf', bbox_inches='tight')
    plt.close()
